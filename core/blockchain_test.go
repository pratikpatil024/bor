...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		if count%2 == 0 {
			receipts, _, usedGas, err = blockchain.processorSerial.Process(block, statedb, vm.Config{})
		} else {
			receipts, _, usedGas, err = blockchain.processorParallel.Process(block, statedb, vm.Config{})
		}
