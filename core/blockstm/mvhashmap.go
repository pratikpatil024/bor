...
func ValidateVersion(txIdx int, lastInputOutput *TxnInputOutput, versionedData *MVHashMap, execTasks taskStatusManager) (valid bool) {
	valid = true
	...
	for _, rd := range lastInputOutput.ReadSet(txIdx) {
		...
		case MVReadResultDone:
			curValid = rd.Kind == ReadKindMap && rd.V == Version{
				TxnIndex:    mvResult.depIdx,
				Incarnation: mvResult.incarnation,
			}
			if !curValid {
				execTasks.addDependencies(mvResult.depIdx, txIdx)
			}
		case MVReadResultDependency:
			curValid = false
		case MVReadResultNone:
			curValid = rd.Kind == ReadKindStorage // feels like an assertion?
		default:
